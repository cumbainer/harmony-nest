...
const fetchData = (spotifyWebApiParam: SpotifyWebApi, searchQueryParam: string) => {
    if (spotifyWebApiParam) {
        if (searchQueryParam.trim().length > 0) {
            return spotifyWebApiParam
                .search(searchQueryParam, ['album', 'artist', 'playlist', 'track'], {limit: 50})
                .then(data => {
    ...
    return (
        <div className="mx-32 relative block">
            ...
            placeholder:text-lg placeholder:mx-10 h-8 text-lg`}
                    placeholder="Search songs" onChange={(event) => {
            ...
            {inputQuery && <div className="h-fit p-3 space-y-1 w-[30rem] bg-[#1B1F38] absolute top-8 z-20">
                <SearchResultsSelections selectedOption={selectedOption} setSelectedOption={setSelectedOption}/>
                <SearchResultList isLoading={isLoading} response={searchResults} selectedOption={selectedOption}/>
            </div>}
        </div>
    );
};
